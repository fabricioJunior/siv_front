...
class EstoqueRelatorioPdfExporter {
  ...
  static Future<void> exportarValorEstoquePorReferencia({
    required List<ProdutoDoEstoquePorReferencia> itens,
    required List<PrecoReferenciaEstoque> precos,
    required String tabelaDePrecoNome,
  }) async {
    final precoPorReferencia = <int, double>{
    ...
    for (final item in itens) {
      ...
      valorTotal += valorUnitario * item.saldoTotal;
    }
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          _cabecalho(
            'Relatório — Valor Total do Estoque por Referência',
            'Tabela de preço: $tabelaDePrecoNome  |  Gerado em: ${_fmtData(DateTime.now())}',
            _azul,
          ),
          pw.SizedBox(height: 12),
          _kpiRow([
            ('Valor total do estoque', _fmtMoeda(valorTotal)),
            ('Referências no relatório', '${itens.length}'),
            ('Referências sem preço na tabela', '$itensSemPreco'),
          ]),
          pw.SizedBox(height: 16),
          pw.Table(
            border: pw.TableBorder.all(color: PdfColors.grey300, width: 0.5),
